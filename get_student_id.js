...
async function check() {
  try {
    const url = `${backend}/api/collections/students/records?filter=user%3D%279zjw4b4ksr394c4%27`;
    ...
    if (res.ok) {
      ...
      console.log('Student records count:', data.totalItems);
      if (data.items.length > 0) {
        console.log('Student record:', JSON.stringify(data.items[0], null, 2));
      }
    }
  } catch (e) {
